...
void generate_cxx_export(DefineStmt* stmt)
{
  ...
  if (opt_gen_c) {
    if (output_header)
      fprintf(output_header, "void yanshi_%s_init(long* start);\n", stmt->lhs.c_str());
    fprintf(output, "void yanshi_%s_init(long* start)\n", stmt->lhs.c_str());
  } else {
    if (output_header)
      fprintf(output_header, "void yanshi_%s_init(long& start);\n", stmt->lhs.c_str());
    fprintf(output, "void yanshi_%s_init(long& start)\n", stmt->lhs.c_str());
  ...
  // yanshi_%s_final
  if (output_header)
      fprintf(output_header, "bool yanshi_%s_is_final(long state);\n", stmt->lhs.c_str());
  fprintf(output, "bool yanshi_%s_is_final(long state)\n", stmt->lhs.c_str());
  fprintf(output, "{\n");
  indent(output, 1);
  ...
  fprintf(output, "};\n");
  indent(output, 1);
  fprintf(output, "const long len = %zd;\n", anno.fsa.finals.size());
  if (opt_gen_c)
    fputs(
"  long l = 0, h = len;\n"
"  while (l < h) {\n"
